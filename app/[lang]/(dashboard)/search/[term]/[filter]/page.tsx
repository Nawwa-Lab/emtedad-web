...
const VALID_FILTERS = ['all', 'namliya', 'khalis', 'qalish', 'members'] as const
            ...
            {MEMBERS.map((m) => (
              <SearchGroup.ListItem key={m.name}>
                <Avatar size="lg" className="bg-sage border border-line-soft">
                  <AvatarFallback className="text-green-deep">{m.av}</AvatarFallback>
                </Avatar>
                <span className="flex-1">
                  <span className="font-cairo font-extrabold text-[14.5px] block">{m.name}</span>
